default export class ModelPath extends AbstractStringAttributeAtom {
	...
	private GridComposite subContainer;
	...
	public AbstractStringAttributeAtom<ModelPath> createAttributeAtomControl(
		...
		contentContainer.setLayoutData(contentgridData);
	...
	@Override
	public AbstractAttributeAtom<ModelPath, String> createAttributeAtomControl(
			Browser browser,
	...
	private void updateContentModel() {

		TreezComposite.clearContent(contentContainer);
		FormToolkit toolkit = new FormToolkit(Display.getCurrent());
		createLabel(contentContainer, toolkit);

		Composite fieldsContainer;

		if (relativeRoot != null) {
			fieldsContainer = new GrabbingRowComposite(contentContainer);
		} else {
			fieldsContainer = new GridComposite(contentContainer);
		}

		GridData gridData = new GridData(SWT.FILL, SWT.FILL, true, false);
		fieldsContainer.setLayoutData(gridData);

		//relative root label
		if (relativeRoot != null) {
			rootLabel = new CustomLabel(toolkit, fieldsContainer, "");
		...
		subContainer = createSubContainer(fieldsContainer);

		GridData subContainerGridData = new GridData(SWT.FILL, SWT.FILL, true, false);
		subContainer.setLayoutData(subContainerGridData);
	...
	private void createLabel(GridComposite parent, FormToolkit toolkit) {
		labelComposite = new CustomLabel(toolkit, parent, label);
		final int prefferedLabelWidth = 80;
		labelComposite.setPrefferedWidth(prefferedLabelWidth);
	}

	@SuppressWarnings("checkstyle:magicnumber")
	private static GridComposite createSubContainer(Composite container) {
	 ...
	 * @param subContainer
	 */
	private void reCreateComboBoxSelector(GridComposite subContainer) {
	 ...
	 * @param subContainer
	 */
	private void reCreateTextFieldAndButtonForTreeSelector(GridComposite subContainer) {
