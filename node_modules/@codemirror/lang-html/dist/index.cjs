...
const autoCloseTags = view.EditorView.inputHandler.of((view, from, to, text) => {
    ...
    let changes = state$1.changeByRange(range => {
        var _a, _b, _c;
        let around = language.syntaxTree(state$1).resolveInner(range.head, -1), name;
        if (around.name == "TagName" || around.name == "StartTag")
            around = around.parent;
        if (text == ">" && around.name == "OpenTag") {
            if (((_b = (_a = around.parent) === null || _a === void 0 ? void 0 : _a.lastChild) === null || _b === void 0 ? void 0 : _b.name) != "CloseTag" && (name = elementName(state$1.doc, around.parent)))
                return { range: state.EditorSelection.cursor(range.head + 1), changes: { from: range.head, insert: `></${name}>` } };
        }
        else if (text == "/" && around.name == "OpenTag") {
            let empty = around.parent, base = empty === null || empty === void 0 ? void 0 : empty.parent;
            if (empty.from == range.head - 1 && ((_c = base.lastChild) === null || _c === void 0 ? void 0 : _c.name) != "CloseTag" && (name = elementName(state$1.doc, base))) {
                let insert = `/${name}>`;
                return { range: state.EditorSelection.cursor(range.head + insert.length), changes: { from: range.head, insert } };
            }
        }
        return { range };
    });
